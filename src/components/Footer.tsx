...
import { Facebook, Twitter, Youtube, Phone, Mail, MapPin, ChevronsUp, MessageCircle, Headphones } from 'lucide-react';
...
const ContactMenu = ({ onCustomerServiceClick }: ContactMenuProps) => {
  ...
  return (
    <div className="relative">
      <motion.button
        ...
        className="flex items-center gap-2 px-5 py-2.5 bg-[#D32F2F] hover:bg-[#B71C1C] text-white font-bold text-sm rounded-2xl shadow-xl border border-white/10"
        ref={buttonRef}
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute bottom-full right-0 mb-3 w-[272px] bg-white rounded-[20px] shadow-[0_12px_40px_rgba(0,0,0,0.12)] p-4 z-[1001]"
            ref={menuRef}
          >
            <div className="flex flex-col gap-1">
              ...
              >
                ...
                <span className="text-[#1F2937] font-semibold text-sm">Chat với nhân viên</span>
              </button>
              <button
                onClick={handleZalo}
                className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl hover:bg-slate-50 transition-colors text-left"
              >
                ...
                <span className="text-[#1F2937] font-semibold text-sm">Liên hệ Zalo</span>
              </button>
            </div>
          </motion.div>
        )}
...
export default function Footer({ onBlogClick, isCartDrawerOpen, isCategoryOpen, onCustomerServiceClick }: FooterProps) {
  ...
  return (
    <footer className="relative bg-brand-secondary text-slate-300 font-sans border-t-[8px] border-brand-primary">
      ...
      <div className="fixed bottom-6 right-6 flex flex-col gap-3 z-[1000] items-end">
        <ContactMenu onCustomerServiceClick={onCustomerServiceClick} />
        <AnimatePresence>
